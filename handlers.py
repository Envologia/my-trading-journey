...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Handle therapy states
        ...
        content.append({"role": "user", "content": update.message.text})
        ...
        try:
            ai_response = ai_therapy.get_therapy_response(update.message.text, user)
    ...
    # Handle journaling states
        ...
        trade = Trade(
            user_id=user.id,
            date=datetime.strptime(state_data.get('date'), '%Y-%m-%d').date(),
            pair_traded=state_data.get('pair'),
            stop_loss=state_data.get('stop_loss'),
            take_profit=state_data.get('take_profit'),
